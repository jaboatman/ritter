...
use ritter_types::grammar::{Grammar, RuleDef};
...
pub fn expand_rule(input: DeriveInput) -> Result<proc_macro2::TokenStream> {
    ...
    ritter_common::expansion::process_rule(d, &mut ctx)?;
    ...
    let (extract, rule) = match input.data {
        Data::Struct(DataStruct { fields, .. }) => {
            ...
            let extract_impl: Item = syn::parse_quote! {
                impl ::ritter::Extract for #ident {
                    type Output = Self;
                    type LeafFn = ();
                    #[allow(non_snake_case)]
                    fn extract<'tree>(
                        ctx: &mut ::ritter::extract::ExtractContext,
                        node: Option<::ritter::tree_sitter::Node<'tree>>,
                        source: &[u8],
                        _l: Self::LeafFn,
                    ) -> Result<Self, ::ritter::extract::ExtractError<'tree>> {
                        let node = node.ok_or_else(|| {
                            ::ritter::error::ExtractError::missing_node(ctx)
                        })?;
                        #extract_expr
                    }
                }
            ...
            let rule_impl: Item = syn::parse_quote! {
                impl ::ritter::rule::Rule for #ident {
        ...
        Data::Enum(DataEnum { variants, .. }) => {
            ...
            let extract_impl: Item = syn::parse_quote! {
                impl ::ritter::Extract for #enum_name {
                    type Output = Self;
                    type LeafFn = ();
                    #[allow(non_snake_case)]
                    fn extract<'tree>(
                        ...
                        if !cursor.goto_first_child() {
                            return Err(::ritter::error::ExtractError::missing_node(ctx));
                        }
                        loop {
                            ...
                            match node.kind() {
                                ...
                                k => if !cursor.goto_next_sibling() {
                                    return Err(::ritter::error::ExtractError::missing_enum(ctx));
                                }
                            }
                        }
                    }
            ...
            let rule_impl: Item = syn::parse_quote! {
                impl ::ritter::rule::Rule for #enum_name {
                    const RULE_NAME: &'static str = #ident_str;
                    fn produce_ast() -> String {
                        String::new()
                    }
    ...
    let lang = if let Some((ident, lang)) = ctx.language_rule {
        ...
        quote! {
            impl ::ritter::rule::Language for #ident {
                fn produce_grammar() -> String {
                    String::new()
                }

                fn language() -> ::ritter::tree_sitter::Language {
                    unsafe extern "C" {
                        fn #tree_sitter_ident() -> ::ritter::tree_sitter::Language;
                    }
                ...
                fn parse(input: &str) -> ::ritter::ParseResult<Self> {
                    ::ritter::__private::parse(input, Self::language)
                }
            }
        }
    } else {
...
fn gen_field(ident_str: Option<&str>, leaf: Field, rule: &RuleDef) -> Result<Expr> {
    ...
    if should_skip {
        ...
        return Ok(syn::parse_quote!({
            ::ritter::__private::skip_text(state, #ident_str)?;
            Ok::<_, ::ritter::extract::ExtractError>(())
        }));
    }
    ...
    let extractor: Expr = parse_quote! { ::ritter::extract::BaseExtractor::default() };
    ...
    Ok(parse_quote! {
        ::ritter::__private::extract_field::<#leaf_type, _>(#extractor, #leaf_fn, state, #extract_state, source, #ident_str)
    })
}
...
fn gen_struct_or_variant(
    ...
    Ok(
        syn::parse_quote!(::ritter::__private::extract_struct_or_variant(
...
fn rule_def_to_extract(def: &RuleDef) -> Result<proc_macro2::TokenStream> {
    ...
    Ok(quote! {
        ::ritter::extract::ExtractFieldContext::new(#num_states, #optional, |state| {
            match state {
                #(#states)*
                #num_states => ::ritter::extract::ExtractFieldState::Complete,
                _ => ::ritter::extract::ExtractFieldState::Overflow,
            }
        })
    })
}
...
fn rule_def_add_state(def: &RuleDef, optional: bool, states: &mut Vec<proc_macro2::TokenStream>) {
    let s = match def {
        RuleDef::SYMBOL { name } => {
            ...
            quote! {
                ::ritter::extract::ExtractFieldState::Str(#name, true, #optional)
            }
        }
        RuleDef::STRING { value } => {
            quote! {
                ::ritter::extract::ExtractFieldState::Str(#value, false, #optional)
            }
        }
        ...
        RuleDef::CHOICE { members } => {
            ...
            if let Some(value) = def.as_optional() {
                ...
                quote! {
                    ::ritter::extract::ExtractFieldState::Choice(&[#(#strs),*], #optional)
                }
            }
        }
        ...
        RuleDef::REPEAT { content } => {
            ...
            quote! {
                ::ritter::extract::ExtractFieldState::Repeat(#value, #named)
            }
        }
        RuleDef::REPEAT1 { content } => match &**content {
            ...
            RuleDef::FIELD { name: _, content } => {
                ...
                quote! {
                    ::ritter::extract::ExtractFieldState::Repeat1
                }
            }
