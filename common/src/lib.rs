...
use quote::ToTokens;
use ritter_types::grammar::RuleDef;
...
impl LanguageExpr {
    pub fn from_attr(a: &Attribute) -> Result<Self> {
        ...
        if path != "language" {
            panic!("Expected language in LanguageExpr, this is a bug in ritter");
        }
...
pub fn sitter_attr_matches(attr: &Attribute, name: &str) -> bool {
    ...
    if path.segments.len() == 1 {
        ...
        path.segments[0].ident == "ritter" && path.segments[1].ident == name
    } else {
        false
    }
}
...
pub fn wrap_leaf_type(ty: &Type, skip_over: &HashSet<&str>) -> Type {
    ...
    if let Type::Path(p) = &mut ty {
        ...
        if skip_over.contains(type_segment.ident.to_string().as_str()) {
            ...
            parse_quote!(::ritter::extract::WithLeaf<#ty, _>)
        }
    } else {
        parse_quote!(::ritter::extract::WithLeaf<#ty, _>)
    }
